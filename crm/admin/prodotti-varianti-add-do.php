...
$querySql =
    ...
    "pr_ct_id, pr_st_id, pr_mr_id, pr_capofila, pr_codice_rif, pr_codice_ean, pr_titolo, pr_abstract, pr_descrizione, pr_prezzo, ".
    "pr_prezzo_scontato, pr_sconto, pr_peso, pr_giacenza, pr_si_id, pr_formato, pr_allegato, pr_immagine, pr_note, pr_timestamp, pr_stato".
    ") VALUES (".
    "'$pr_ct_id', '$pr_st_id', '$pr_mr_id', '$pr_id', '$pr_codice_rif', '$pr_codice_ean', '$pr_titolo', '$pr_abstract', '$pr_descrizione', '$pr_prezzo', ".
    "'$pr_prezzo_scontato', '$pr_sconto', '$pr_peso', '$pr_giacenza', '$pr_si_id', '$pr_formato', '$pr_allegato', '$pr_immagine', '$pr_note', '$pr_timestamp', 1) ";
...
/*
$querySqlImg = "SELECT pr_immagine FROM pr_prodotti WHERE pr_id = '$last_id' ";
$resultImg = $dbConn->query($querySqlImg);
$row_data = $resultImg->fetch_assoc();
$pr_immagine = $row_data['pr_immagine'];
$resultImg->close();

if (strlen($pr_immagine)==0)
{
    $img = ", pr_immagine = $pr_immagine ";
}
else{ $img="";}*/
