...
$querySql = "SELECT *, COUNT(or_id) AS or_colli, CONCAT(ut_cognome, ' ', ut_nome) AS ut_nominativo, SUM(or_pr_prezzo * or_pr_quantita) As or_totale FROM or_ordini INNER JOIN ut_utenti ON ut_codice = or_ut_codice WHERE or_stato_export > 0 GROUP BY or_codice ORDER BY or_codice ";
...
while (($row_data = $result->fetch_assoc()) !== NULL) {
    ...
    $list[] = array(
        ...
        $row_data['ut_nominativo'], //ragione sociale destinatario lunga max 35 caratteri
        $row_data['ut_ragione_sociale'], //ragione sociale destinatario aggiuntiva lunga max 35 caratteri (opzionale)
        $row_data['ut_indirizzo'], //indirizzo di destinazione lungo max 35 caratteri
        '', //Pudo ID per il servizio Fermopoint
        $row_data['ut_cap'], //cap di destinazione
        $row_data['ut_citta'], //località di destinazione
        $row_data['ut_provincia'], //provincia di destinazione
        ...
        $row_data['ut_telefono'], //telefono (opzionale)
        ...
        $row_data['ut_email'], //e-mail (opzionale)
        $row_data['ut_telefono'], //cellulare (opzionale)
