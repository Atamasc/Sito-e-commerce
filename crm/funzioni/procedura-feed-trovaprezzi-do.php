...
$querySql = "SELECT * FROM pr_prodotti INNER JOIN mr_marchi ON mr_codice = pr_codice_marchio WHERE pr_stato > 0 AND pr_esistenza > 0 AND mr_riservato = 0 ORDER BY pr_timestamp ";
$result = $dbConn->query($querySql);

while ($row_data = $result->fetch_array()) {

    $pr_id = $row_data['pr_id'];
    $get_pr_codice_marchio = $row_data['pr_codice_marchio'];
    ...
    $feed_trovaprezzi = fopen("../../ftp/trovaprezzi.txt","a");
    fwrite($feed_trovaprezzi, $row_data['pr_descrizione_breve']."|".$row_data['pr_descrizione_marchio']."|".$row_data['pr_descrizione_estesa']."|".$row_data['pr_prezzo']."|".$row_data['pr_prezzo_scontato']."|".$row_data['pr_codice']."|".$URL."
    |".$row_data['pr_esistenza']."|".$row_data['pr_descrizione_marchio'].";".$row_data['pr_descrizione_linea'].";".$alberatura."|".$URL_immagine."|".$spedizione."|".$vuoto."|".$row_data['pr_codice_ean']."|".$vuoto."|".$vuoto."|".$vuoto."| <endrecord> \r\n");

    /*
    $data[] = $row_data['pr_codice_ean'].";".$row_data['pr_descrizione_marchio'].";".$row_data['pr_descrizione_breve'].
...
$querySql = "INSERT INTO lf_log_feed(lf_titolo, lf_tipo, lf_data, lf_record) VALUES ('Trovaprezzi', 'trovaprezzi', '$lf_data', '$count')";
$result = $dbConn->query($querySql);
